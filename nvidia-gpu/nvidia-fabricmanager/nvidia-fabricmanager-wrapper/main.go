...
const (
	...
	smStopTimout    = 5 * time.Second
	smSocketTimeout = 15 * time.Second
)
...
func main() {
	...
	if fmSmMgmtPortGUID != "" {
		...
		runCommand(runCtx, &cmdWg, gracefulShutdown, smStopTimout, smCmdFile, "--config", smConfigFile,
		...
		err := waitForFile(runCtx, smSocket, smSocketTimeout)
		if err != nil {
